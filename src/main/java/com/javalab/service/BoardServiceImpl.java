 ...
 *  - 메소드에 별도로 트랜잭션을 안 걸어도 모든 메소드에 트랜잭션이 적용된다.
 *    즉, 모든 메소드를 실행하다가 (체크드)예외 발생 시 자동 롤백된다.
 *  - 만약 메소드에 별도로 트랜잭션이 적용되어 있으면 그 설정이 우선한다.
 *  - 클래스 전체에는 @Transactional을 안 붙이고 필요한 메소드에만 해줌.
 * @author 505
 *
 */
...
public class BoardServiceImpl implements BoardService {
	...
	public List<Board> getBoardList(Board vo) {
		...
		List<Board> boardList = boardDAO.getBoardList(vo);
	...
	public int insertBoard(Board vo) {
		...
		/*
		 * vo에는 전달받은 값과 자동으로 채번된 번호까지 같이 저장되어 있다. (편리한 기능)
		 * 마이바티스 저장 시 시퀀스로 따진 번호를 여기에 넣어준다.
		 * 개발자가 이 알고리즘을 구현하려면 상당한 시간이 걸린다.
		 */
		result = boardDAO.insertBoard(vo);
		
		log.info("저장 후 result : " + result);
	...
	@Override
	public void updateHit(Board vo) {
